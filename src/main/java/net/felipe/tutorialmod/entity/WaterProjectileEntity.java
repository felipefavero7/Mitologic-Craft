...
public class WaterProjectileEntity extends ThrownItemEntity {
    ...
    public WaterProjectileEntity(EntityType<? extends ThrownItemEntity> entityType, World world) {
        super(entityType, world);
    }

    public WaterProjectileEntity(World world, LivingEntity owner) {
        super(EntityMod.WaterprojectileEntityType, owner, world);
    }

    public WaterProjectileEntity(World world, double x, double y, double z) {
        super(EntityMod.WaterprojectileEntityType, x, y, z, world);
    }
    ...
    private ParticleEffect getParticleParameters() {
        ...
        return (ParticleEffect)(itemStack.isEmpty() ? ParticleTypes.DRIPPING_WATER : new ItemStackParticleEffect(ParticleTypes.ITEM, itemStack)); //PLACEHOLDER
    }
    ...
    public void tick() {
        ...
        if (this.isTouchingWater()) {
            for(int i = 0; i < 4; ++i) {
                float g = 0.25F;
                this.getWorld().addParticle(ParticleTypes.RAIN, d - vec3d.x * 0.25D, e - vec3d.y * 0.25D, f - vec3d.z * 0.25D, vec3d.x, vec3d.y, vec3d.z);
            }
        ...
        if (this.getWorld().isWater(bp)) {
            this.getWorld().setBlockState(bp, Blocks.WATER.getDefaultState());
        }
